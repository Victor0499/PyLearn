    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans">
      ...
      <header className="h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-50">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button onClick={logout} className="p-2 text-slate-500 hover:text-red-400 hover:bg-red-500/10 rounded-full transition-colors">
      ...
      <main className="max-w-6xl mx-auto px-6 lg:px-10 py-10">
        ...
        <button
          ...
          className="flex items-center gap-2 text-slate-400 hover:text-indigo-400 transition-colors text-sm font-medium mb-8 group"
        >
          ...
          <>
            ...
            <div className="mb-10">
              <div className="flex items-center gap-3 mb-2">
                <h2 className="text-3xl font-bold text-white">{data.classroom.name}</h2>
              ...
              <p className="text-slate-400">Progreso detallado de los estudiantes inscritos en esta clase.</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold text-white">{avgCompleted}</p>
                  <p className="text-xs text-slate-400">Promedio de ejercicios completados</p>
                </div>
              </div>
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex items-center gap-4">
                ...
                <div>
                  <p className="text-lg font-bold text-white truncate">{mostActive?.username ?? "—"}</p>
                  <p className="text-xs text-slate-400">Estudiante más activo</p>
                </div>
              </div>
            </div>
            ...
            <section>
              ...
              {data.students.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-slate-700 bg-slate-900/40 p-16 text-center">
                  ...
                  <p className="text-slate-400 font-medium">Ningún estudiante se ha unido a esta clase aún.</p>
                  <p className="text-slate-500 text-sm mt-1">Comparte el código <span className="font-mono text-indigo-400">{data.classroom.code}</span> para que se inscriban.</p>
                </div>
              ) : (
                    ...
                    .map((student, idx) => {
                      ...
                      return (
                        ...
                        >
                          {/* Fila principal */}
                          <div
                            className="flex items-center justify-between p-5 cursor-pointer hover:bg-slate-800/40 transition-colors"
                          ...
                          >
                            <div className="flex items-center gap-4 min-w-0">
                              ...
                              <div className="min-w-0">
                                <p className="font-semibold text-white truncate">{student.username}</p>
                                <p className="text-xs text-slate-500 truncate">{student.email}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-6 shrink-0 ml-4">
                              <div className="text-right hidden sm:block">
                                <p className="text-lg font-bold text-white">{student.total_exercises_completed}</p>
                          ...
                          {/* Detalle expandido */}
                          {isExpanded && (
                            <div className="border-t border-slate-800 px-5 pb-5 pt-4 bg-slate-950/40">
                              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3">
                                Lecciones con progreso registrado
                              </p>
                              {student.lessons.length === 0 ? (
                                <p className="text-slate-500 text-sm">Este estudiante aún no ha completado ningún ejercicio.</p>
                              ) : (
                                  ...
                                  {student.lessons.map((lesson) => (
                                    ...
                                    >
                                      <div className="flex items-center justify-between mb-1">
                                        ...
                                        <span className="text-xs font-bold text-white">{lesson.exercises_done} ej.</span>
                                      </div>
