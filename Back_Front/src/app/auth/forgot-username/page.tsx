...
import { BookOpen, ArrowLeft, Mail, AlertTriangle, CheckCircle, User } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.isGoogle) {
        ...
        setSuccessMessage(data.message || "Si el correo está registrado, recibirás tu usuario en breve.");
      }
    } catch (err: any) {
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-2xl p-8 shadow-2xl">
          <div className="flex items-center space-x-2 mb-6">
            <Link href="/login" className="text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors">
              <ArrowLeft className="w-5 h-5" />
            </Link>
            <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Recuperar Usuario</h2>
          </div>
          ...
          {isGoogleAccount ? (
              ...
              <div className="p-4 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 text-sm flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-slate-700 dark:text-slate-300">
                    Tu cuenta está registrada a través de Google. Tu nombre de usuario en PyLearn es el nombre de tu cuenta de Google.
                  </p>
                  <p className="text-slate-500 dark:text-slate-400 mt-2 text-xs">
                    Inicia sesión con el botón de Google para acceder a tu cuenta.
                  </p>
                </div>
              </div>
              <Link
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-slate-700 dark:text-slate-300">
                    Si el correo ingresado está registrado en PyLearn, te hemos enviado un correo con tu nombre de usuario. Revisa tu bandeja de entrada o la carpeta de spam.
                  </p>
                </div>
              </div>
              <Link
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed mb-4">
                Ingresa el correo electrónico con el que te registraste y te enviaremos tu nombre de usuario.
              </p>
              <div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-medium rounded-lg shadow-lg shadow-blue-500/20 disabled:opacity-50 transition-all mt-2"
              >
                {loading ? (
